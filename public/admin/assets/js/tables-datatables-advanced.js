...
if (dt_adv_filter_table) {
  const thead = dt_adv_filter_table.querySelector('thead');
  const firstRowCells = thead ? thead.querySelector('tr').children : [];
  ...
  // Add your custom column for the first column (e.g., for controls or actions) if needed
  columns.unshift({
    className: 'control',
    orderable: false,
    targets: 0,
    render: function (data, type, full, meta) {
      return ''; // Custom rendering for the control column
    }
  });
